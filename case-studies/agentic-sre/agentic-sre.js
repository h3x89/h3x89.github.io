  ...
  const layersModel = [
    {
      ...
      role: "The reference environment runs approximately 70 containerized workloads across ingress, observability, automation, AI tooling, storage, backup, developer tooling and runtime applications.",
      how: "Workloads are grouped by operational responsibility rather than by team. The observability stack covers the environment, and changes to its configuration and automation move through the same reviewed delivery path described in this page.",
      why: "It shows the operating model applied to a heterogeneous platform rather than to a single agent workflow or an isolated automation script.",
      evidence: "Stack overview, per-service documentation, runtime configuration and monitoring coverage.",
      html: `<div><h3>Operational Scale</h3><span class="scale-number">~70 containerized workloads</span><p>The reference environment this operating model is exercised against.</p></div><div><ul><li><strong>Domains:</strong> ingress, observability, automation, AI tooling, storage, backup, developer tooling and runtime applications.</li><li><strong>Scope:</strong> one shared change path across the environment, rather than a single scripted workflow.</li></ul><div class="layer-note">Enough variety that a single-purpose automation would not hold — which is the condition the operating model is designed for.</div></div>`,
      scale: true,
      selected: true
    },
    {
